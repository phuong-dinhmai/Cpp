...
struct Node{
	...
	Node (ll x, ll y, ll z, ll t, ll k){
		this->lazy = x; this->left = y; this->right = z; this->sum = t; this->ans = k;
	}
	
	Node() : lazy(0), left(0), right(0), sum(0), ans(0){};
...
Node query(int id, int l, int r, int u, int v){
	if (l > v || r < u) return Node(0, 0, 0, 0, -inf);
